...
RelayFabric's Meshtastic plugin consumes the **MQTT JSON gateway**, not the serial device (the official Meshtastic code is GPL and is never linked). So a Meshtastic node's traffic must reach an MQTT broker the plugin reads. Two ways in:
