...
!!! danger "There is no daemon-layer authentication or authorization"
    ...
    - **Real authentication and authorization — TLS, WebAuthn/passkeys, RBAC, session expiration, audit logging, and role separation that keeps identity-linking permissions apart from route-management permissions — is explicitly the job of the separate `relayfabric-ui` service, fronting this socket. That service is not built yet.** Until it exists, do not expose this admin socket to anyone you wouldn't give a shell on this host.
