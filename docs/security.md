...
!!! danger "There is no daemon-layer authentication or authorization"
    ...
    - **Real authentication and authorization (TLS, WebAuthn/passkeys, RBAC, session expiration, audit logging, and role separation that keeps identity-linking permissions apart from route-management permissions) is explicitly the job of the separate `relayfabric-ui` service, fronting this socket, and as of v0.4 it provides it: **passkey (WebAuthn) authentication with scoped roles** (viewer / operator / route-admin / identity-admin / security-admin / administrator), enforced per method + path on every proxied request. Identity-link data is readable only by identity-admin/administrator. Correlation data is more sensitive than routing config. The RP verification is a clean-room implementation (`relayfabric-ui/src/auth.rs`; the stock crate is MPL-2.0, outside the permissive-only dependency policy) covering ES256 + Ed25519 assertions and `attestation: "none"` registration.** The daemon socket itself remains filesystem-only by design; remote (non-localhost) UI use requires fronting TLS, since browsers only offer WebAuthn in a secure context.
