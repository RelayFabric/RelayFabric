...
mod tests {
    ...
    #[tokio::test]
    async fn federation_envelope_fields_do_not_affect_relay_ipc_goldens() {
        // Cycle F (design §2) added `origin`/`attestations`/`hops` to
        // relay_core::Envelope. Neither PluginToDaemon nor DaemonToPlugin
        // embeds an Envelope directly, so nothing here should have moved --
        // but this crate is the plugin/daemon wire-format boundary golden
        // tests live in, so it re-asserts both golden hexes byte-identical
        // in the SAME test run as this task's change (cycle-D
        // `..._goldens_are_unaffected` precedent), rather than relying on
        // the pre-existing golden tests happening to still be green.
        let mut hello_buf = Vec::new();
        write_frame(&mut hello_buf, &PluginToDaemon::Hello {
            plugin: "lxmf".into(),
            version: "0.1.0".into(),
