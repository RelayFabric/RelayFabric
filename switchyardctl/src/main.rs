    ...
    match args.first().map(String::as_str) {
        ...
        Some("config") => match args.get(1).map(String::as_str) {
            ...
            Some("validate") => match args.get(2) {
                Some(file) => Ok((
                    "POST",
                    "/v1/config/validate".into(),
                    Some(read_config_file(file)?),
                )),
            ...
            _ => {
                Err("usage: switchyardctl config show|validate <file>|apply <file>|rollback".into())
            }
        },
...
mod tests {
    ...
    fn maps_commands_to_paths() {
        assert_eq!(
            request_for(&["status".into()]).unwrap(),
            ("GET", "/v1/status".into(), None)
        );
        assert_eq!(
            request_for(&["plugins".into()]).unwrap(),
            ("GET", "/v1/plugins".into(), None)
        );
        assert_eq!(
            request_for(&["routes".into()]).unwrap(),
            ("GET", "/v1/routes".into(), None)
        );
        assert_eq!(
            request_for(&["queue".into()]).unwrap(),
            ("GET", "/v1/queue".into(), None)
        );
        assert_eq!(
            request_for(&[
                "trace".into(),
                "01890000-0000-7000-8000-000000000000".into()
            ])
            .unwrap(),
            (
                "GET",
                "/v1/messages/01890000-0000-7000-8000-000000000000".into(),
                None
            )
        );
        assert_eq!(
            request_for(&["identities".into()]).unwrap(),
            ("GET", "/v1/identities".into(), None)
        );
        assert_eq!(
            request_for(&["federation".into()]).unwrap(),
            ("GET", "/v1/federation".into(), None)
        );
        assert_eq!(
            request_for(&["discovery".into()]).unwrap(),
            ("GET", "/v1/discovery".into(), None)
        );
        assert_eq!(
            request_for(&["openapi".into()]).unwrap(),
    ...
    fn link_requires_requester_target_and_at_least_one_display_name_word() {
        ...
        assert!(
            request_for(&["link".into(), "a:b".into(), "c:d".into()]).is_err(),
    ...
    fn unlink_builds_a_delete_with_no_body() {
        assert_eq!(
            request_for(&["unlink".into(), "42".into()]).unwrap(),
    ...
    fn config_show_builds_a_get_with_no_body() {
        assert_eq!(
            request_for(&["config".into(), "show".into()]).unwrap(),
    ...
    fn config_validate_requires_a_file_argument_and_errors_on_a_missing_file() {
        ...
        assert!(request_for(&[
            "config".into(),
            "validate".into(),
            "/nonexistent/relayfabric-ctl-test.yaml".into()
        ])
        .is_err());
    }
    ...
    fn config_rollback_builds_a_post_with_no_body() {
        assert_eq!(
            request_for(&["config".into(), "rollback".into()]).unwrap(),
    ...
    fn body_of_accepts_only_the_expected_status() {
        ...
        assert!(
            body_of(raw, "200").is_err(),
            "202 must not satisfy a 200 expectation"
        );
    }
    ...
    fn docs_message_includes_socket_socat_oneliner_and_ssh_alternative() {
        ...
        assert!(
            msg.contains("socat TCP-LISTEN:8099,fork UNIX-CONNECT:/var/lib/relayfabric/admin.sock")
        );
        assert!(msg.contains("http://localhost:8099/docs"));
        assert!(msg.contains("ssh -L 8099:/var/lib/relayfabric/admin.sock"));
        assert!(
            msg.contains("switchyardctl openapi"),
            "must note the headless alternative"
        );
    }
