...
pub fn evaluate<'a>(policies: &'a [Policy], env: &Envelope, dest: &Endpoint) -> Decision<'a> {
...
mod tests {
    ...
    fn deny_and_drop_kinds_deny() {
        ...
        assert!(matches!(
            evaluate(&[deny], &env("text"), &dest()),
            Decision::Deny { .. }
        ));
        let strip = policy(
            &["mockb"],
            PolicyRules {
                drop_kinds: vec!["location".into()],
                ..Default::default()
            },
        );
        assert!(matches!(
            evaluate(std::slice::from_ref(&strip), &env("location"), &dest()),
            Decision::Deny { .. }
        ));
        assert!(matches!(
            evaluate(&[strip], &env("text"), &dest()),
            Decision::Allow { .. }
        ));
    }
    ...
    fn max_attachment_bytes_takes_the_minimum_across_matching_policies() {
        let a = policy(
            &["mockb"],
            PolicyRules {
                max_attachment_bytes: Some(5_000_000),
                ..Default::default()
            },
        );
        let b = policy(
            &["mockb"],
            PolicyRules {
                max_attachment_bytes: Some(1_000_000),
                ..Default::default()
            },
        );
        match evaluate(&[a, b], &env("text"), &dest()) {
