...
/// A minimal, valid starter config `switchyardd init` writes for a first-time
/// operator: a named node, a short absolute data_dir, no plugins/routes yet
/// (deny-by-default), and the standard lifecycle defaults. `--check-config`
/// passes on it as-is; the operator uncomments a plugin and adds a route.
const STARTER_CONFIG: &str = r#"# RelayFabric config — generated by `switchyardd init`.
# Full reference: docs/relayfabric.example.yaml and docs/configuration.md.
node:
  name: my-relayfabric
  # Keep this a SHORT absolute path (the plugin/admin socket paths derive
  # from it and must fit the ~104-byte Unix-socket limit):
  data_dir: /var/lib/relayfabric

# Bridges to other networks. Each plugin is a separate supervised process;
# see docs/plugins.md. Example (commented) — enable and configure one:
plugins: {}
  # mqtt:
  #   enabled: true
  ...
  #     client_id: relayfabric

# Routes bridge one plugin's endpoints to another (deny-by-default: nothing
# flows until a route says so). Example:
routes: []
  # - name: example
...
/// `switchyardd init [--config <path>]`: write a starter config if none
/// exists (never overwrites), then print next steps.
fn run_init(args: &[String]) {
    ...
    if path.exists() {
        eprintln!("init: {config_path} already exists; refusing to overwrite");
        std::process::exit(1);
    }
    if let Some(parent) = path.parent() {
        if !parent.as_os_str().is_empty() {
            if let Err(e) = std::fs::create_dir_all(parent) {
                eprintln!("init: cannot create {}: {e}", parent.display());
                std::process::exit(1);
            }
        }
    }
    if let Err(e) = std::fs::write(path, STARTER_CONFIG) {
        eprintln!("init: cannot write {config_path}: {e}");
        std::process::exit(1);
    }
    println!("wrote starter config to {config_path}");
    println!("next: edit it (enable a plugin + add a route), then");
    println!("      switchyardd --check-config --config {config_path}");
}
...
mod tests {
    ...
    fn starter_config_is_valid() {
        // `switchyardd init`'s output must pass --check-config as-is.
        let cfg = config::load_from_str(STARTER_CONFIG).expect("starter config must parse");
        config::validate(&cfg).expect("starter config must validate");
